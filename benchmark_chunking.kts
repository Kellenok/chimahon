...
fun main() {
    ...
    val images = testDir.listFiles()?.filter { it.extension == "webp" }?.sorted() ?: return
    ...
    for (imageFile in images) {
        ...
        // Quality: check variance at each cut point
...
fun findCutsVariance(img: BufferedImage, width: Int, height: Int): List<Int> {
    ...
    while (currentY < height) {
        ...
        val targetChunkHeight = minOf(CHUNK_HEIGHT_LIMIT, remainingHeight)
        ...
        val searchStart = maxOf(0, targetY - CHUNK_BOUNDARY_SEARCH_RANGE)
        val searchEnd = minOf(height - CHUNK_BOUNDARY_LINE_MIN_HEIGHT, targetY + CHUNK_BOUNDARY_SEARCH_RANGE)
        ...
        for (y in searchStart..searchEnd - CHUNK_BOUNDARY_LINE_MIN_HEIGHT + 1) {
            ...
            if (rowVariance < LOW_VARIANCE_THRESHOLD) {
                var consecutive = 1
                for (cy in y + 1 until minOf(y + CHUNK_BOUNDARY_LINE_MIN_HEIGHT, searchEnd + 1)) {
...
fun findCutsEdge(img: BufferedImage, width: Int, height: Int): List<Int> {
    ...
    while (currentY < height) {
        ...
        for (y in searchStart..searchEnd - CHUNK_BOUNDARY_LINE_MIN_HEIGHT + 1) {
            // Check if this row AND next minLineHeight rows all have low edge scores
            var allLow = true
