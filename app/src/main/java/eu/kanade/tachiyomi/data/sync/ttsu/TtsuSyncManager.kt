...
class TtsuSyncManager(private val context: Context) {
    ...
    suspend fun pushProgressToGoogleDrive(
        ...
        try {
            ...
            logcat(LogPriority.ERROR, tag = "TTSU-SYNC", throwable = e) { "Failed to sync TTSU progress: ${e.message}" }
            false
        }
    }
    ...
    suspend fun pullBookProgress(book: com.canopus.chimareader.data.BookMetadata): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            logcat(LogPriority.ERROR, throwable = e) { "Failed to pull progress for $title: ${e.message}" }
            false
        }
    }
    ...
    suspend fun pushStatisticsToGoogleDrive(
        ...
        try {
            ...
            logcat(LogPriority.ERROR, tag = "TTSU-SYNC", throwable = e) { "Failed to sync TTSU statistics: ${e.message}" }
            false
        }
    }
    ...
    suspend fun pullStatistics(book: BookMetadata): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            logcat(LogPriority.INFO, tag = "TTSU-SYNC") { "pullStatistics: merged for '$title'" }
            return@withContext true
        } catch (e: Exception) {
            logcat(LogPriority.ERROR, throwable = e) { "Failed to pull statistics for $title: ${e.message}" }
            false
        }
    }
